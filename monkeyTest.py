...
def runnerPool():
    ...
    if devices:
        ...
        # pool.apply_async(test,(devices, ))
        pool.map(start, devices_Pool)
        # pool.map(test, devices_Pool)
        pool.close()
        pool.join()
    else:
        print("No devices")
...
def start(devicess):
    ...
    print(mc["cmd"])
    start_monkey("adb -s " + devices + " " + mc["cmd"], mc["log"])
    time.sleep(1)
    starttime = datetime.datetime.now()
    pid = BaseMonitor.get_pid(mc["package_name"], devices)
    # pid = ba.get_app_pid(devices,mc["package_name"])
    ...
    while True:
        with open(mc["monkey_log"], encoding='utf-8') as monkeylog:
            ...
            if monkeylog.read().count('Monkey is over!') > 0:
                endtime = datetime.datetime.now()
                print(str(devices)+"Test has finish!!!!!")
                ...
                app[devices]["header"]["time"] = str((endtime - starttime).seconds) + "S"
